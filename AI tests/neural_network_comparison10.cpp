/**
 * Neural Network Training Comparison #10: FISHER INFORMATION ELIMINATION
 * 
 * Strategy: Eliminate weights with lowest Fisher Information
 *   - Fisher Information = E[(d log p / d theta)^2]
 *   - Approximated as: gradient^2 (squared gradient magnitude)
 *   - Low Fisher Info = weight carries little information about output
 *   - N=3: Eliminate 33% lowest Fisher info weights (Quaylyn's optimal)
 *   - N=9: Eliminate 11% lowest Fisher info weights (conservative)
 * 
 * This tests Quaylyn's Law on INFORMATION-THEORETIC elimination
 */
...
constexpr int FISHER_ACCUMULATION_EPOCHS = 10; // Epochs to accumulate Fisher info
...
// ============================================================================
// NEURAL NETWORK WITH FISHER INFORMATION TRACKING
// ============================================================================
...
struct NeuralNetwork {
    ...
    // Fisher Information accumulators (sum of squared gradients)
    std::vector<std::vector<double>> fisher_ih;
    std::vector<std::vector<double>> fisher_ho;
    int fisherSampleCount;
    ...
    NeuralNetwork() {
        ...
        // Fisher Information
        fisher_ih.resize(HIDDEN_NEURONS, std::vector<double>(INPUT_SIZE, 0.0));
        fisher_ho.resize(OUTPUT_SIZE, std::vector<double>(HIDDEN_NEURONS, 0.0));
        fisherSampleCount = 0;
    ...
    void resetFisherInfo() {
        for (auto& row : fisher_ih) std::fill(row.begin(), row.end(), 0.0);
        for (auto& row : fisher_ho) std::fill(row.begin(), row.end(), 0.0);
        fisherSampleCount = 0;
    }
...
// ============================================================================
// FISHER INFORMATION ELIMINATION TRAINER
// ============================================================================

struct FisherInformationTrainer {
    double eliminationRate;
    
    FisherInformationTrainer(double elimRate) : eliminationRate(elimRate) {}
    
    void trainAndAccumulateFisher(NeuralNetwork& nn, const std::vector<DataPoint>& data, double lr) {
        std::vector<std::vector<double>> grad_ih(HIDDEN_NEURONS, 
            std::vector<double>(INPUT_SIZE, 0.0));
        std::vector<std::vector<double>> grad_ho(OUTPUT_SIZE, 
            std::vector<double>(HIDDEN_NEURONS, 0.0));
        ...
        // Per-sample gradient squared accumulation for Fisher
        for (const auto& dp : data) {
            nn.forward(dp.input);
            ...
            for (int i = 0; i < OUTPUT_SIZE; i++) {
                for (int j = 0; j < HIDDEN_NEURONS; j++) {
                    double g = output_delta[i] * nn.hidden_act[j];
                    grad_ho[i][j] += g;
                    // Accumulate squared gradient for Fisher Information
                    if (nn.mask_ho[i][j] > 0.5) {
                        nn.fisher_ho[i][j] += g * g;
                    }
                }
                grad_bo[i] += output_delta[i];
            }
            ...
            for (int i = 0; i < HIDDEN_NEURONS; i++) {
                for (int j = 0; j < INPUT_SIZE; j++) {
                    double g = hidden_delta[i] * dp.input[j];
                    grad_ih[i][j] += g;
                    // Accumulate squared gradient for Fisher Information
                    if (nn.mask_ih[i][j] > 0.5) {
                        nn.fisher_ih[i][j] += g * g;
                    }
                }
                grad_bh[i] += hidden_delta[i];
            }
        ...
        nn.fisherSampleCount += data.size();
    ...
    void eliminateLowFisherWeights(NeuralNetwork& nn) {
        if (nn.fisherSampleCount == 0) return;
        
        // Collect all weights with their Fisher Information
        std::vector<std::tuple<double, int, int, int>> fisherScores; // (fisher, layer, i, j)
        
        for (int i = 0; i < HIDDEN_NEURONS; i++) {
            for (int j = 0; j < INPUT_SIZE; j++) {
                if (nn.mask_ih[i][j] > 0.5) {
                    // Average Fisher Information
                    double fisher = nn.fisher_ih[i][j] / nn.fisherSampleCount;
                    fisherScores.push_back({fisher, 0, i, j});
                }
            }
        }
        ...
        for (int i = 0; i < OUTPUT_SIZE; i++) {
            for (int j = 0; j < HIDDEN_NEURONS; j++) {
                if (nn.mask_ho[i][j] > 0.5) {
                    double fisher = nn.fisher_ho[i][j] / nn.fisherSampleCount;
                    fisherScores.push_back({fisher, 1, i, j});
                }
            }
        }
        
        if (fisherScores.empty()) return;
        
        // Sort by Fisher Information (lowest first = least important)
        std::sort(fisherScores.begin(), fisherScores.end(),
            [](const auto& a, const auto& b) { return std::get<0>(a) < std::get<0>(b); });
        
        // Eliminate lowest Fisher Information weights
        int toEliminate = std::max(1, static_cast<int>(fisherScores.size() * eliminationRate));
        toEliminate = std::min(toEliminate, static_cast<int>(fisherScores.size()) - 4);
        
        for (int i = 0; i < toEliminate; i++) {
            auto [fisher, layer, row, col] = fisherScores[i];
            if (layer == 0) {
    ...
    void train(NeuralNetwork& nn, const std::vector<DataPoint>& data, 
        ...
        trainAndAccumulateFisher(nn, data, LEARNING_RATE);
        
        // Periodic elimination
        if ((epoch + 1) % ELIMINATION_INTERVAL == 0 && epoch < MAX_EPOCHS - 50) {
            eliminateLowFisherWeights(nn);
        }
...
TrainingMetrics trainWithFisherElimination(const std::vector<DataPoint>& trainData,
                                            const std::vector<DataPoint>& testData,
                                            double elimRate, ProgressBar* progress) {
    TrainingMetrics metrics;
    NeuralNetwork nn;
    FisherInformationTrainer trainer(elimRate);
...
int main() {
    std::cout << "\n";
    std::cout << "╔════════════════════════════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║       NEURAL NETWORK TRAINING #10: FISHER INFORMATION ELIMINATION                         ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════════════════════════════════════╝\n\n";
    
    std::cout << "Strategy: Eliminate weights with lowest Fisher Information\n";
    std::cout << "  • Fisher Info ≈ E[gradient²] = information about output\n";
    std::cout << "  • Low Fisher = weight carries little predictive information\n";
    std::cout << "  • N=3: Eliminate 33% lowest Fisher (Quaylyn's optimal)\n";
    std::cout << "  • N=9: Eliminate 11% lowest Fisher (conservative)\n";
    ...
    for (int trial = 0; trial < NUM_TRIALS; trial++) {
        ...
        auto n3Metrics = trainWithFisherElimination(trainData, testData, 
        ...
        auto n9Metrics = trainWithFisherElimination(trainData, testData, 
    ...
    // Print results
    std::cout << "╔════════════════════════════════════════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║                              RESULTS: FISHER INFORMATION ELIMINATION                                   ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "\n  FISHER INFORMATION ELIMINATION PRINCIPLE:\n";
    std::cout << "  • Fisher Information measures parameter importance\n";
    std::cout << "  • F(θ) = E[(∂log p(y|x,θ)/∂θ)²] ≈ E[gradient²]\n";
    std::cout << "  • Low Fisher = weight doesn't affect predictions much\n";
    std::cout << "  • N=3 (33%): Aggressive information-based pruning\n";
