/**
 * Neural Network Training Comparison #9: GRADIENT VARIANCE ELIMINATION
 * 
 * Strategy: Eliminate weights with high gradient variance (unstable gradients)
 *   - Track gradient history for each weight
 *   - High variance = gradient oscillates = weight is "undecided"
 *   - N=3: Eliminate 33% highest variance weights (Quaylyn's optimal)
 *   - N=9: Eliminate 11% highest variance weights (conservative)
 * 
 * This tests Quaylyn's Law on STABILITY-BASED elimination
 */
...
constexpr int GRADIENT_HISTORY_SIZE = 20;      // Track last N gradients for variance
...
double calculateVariance(const std::deque<double>& values) {
    ...
    return (sumSq / values.size()) - (mean * mean);
}
...
// ============================================================================
// NEURAL NETWORK WITH GRADIENT HISTORY
// ============================================================================
...
struct NeuralNetwork {
    ...
    // Gradient history for variance tracking
    std::vector<std::vector<std::deque<double>>> grad_history_ih;
    std::vector<std::vector<std::deque<double>>> grad_history_ho;
    
    // Mask for eliminated weights
    ...
    NeuralNetwork() {
        ...
        // Gradient history
        grad_history_ih.resize(HIDDEN_NEURONS, 
            std::vector<std::deque<double>>(INPUT_SIZE));
        grad_history_ho.resize(OUTPUT_SIZE, 
            std::vector<std::deque<double>>(HIDDEN_NEURONS));
    ...
    void clearGradientHistory() {
        for (auto& row : grad_history_ih) {
            for (auto& dq : row) dq.clear();
        }
        for (auto& row : grad_history_ho) {
            for (auto& dq : row) dq.clear();
        }
    }
...
// ============================================================================
// GRADIENT VARIANCE ELIMINATION TRAINER
// ============================================================================

struct GradientVarianceTrainer {
    double eliminationRate;
    
    GradientVarianceTrainer(double elimRate) : eliminationRate(elimRate) {}
    
    void trainAndTrackGradients(NeuralNetwork& nn, const std::vector<DataPoint>& data, double lr) {
        std::vector<std::vector<double>> grad_ih(HIDDEN_NEURONS, 
            std::vector<double>(INPUT_SIZE, 0.0));
        std::vector<std::vector<double>> grad_ho(OUTPUT_SIZE, 
            std::vector<double>(HIDDEN_NEURONS, 0.0));
        ...
        // Update weights AND track gradient history
        for (int i = 0; i < HIDDEN_NEURONS; i++) {
            for (int j = 0; j < INPUT_SIZE; j++) {
                if (nn.mask_ih[i][j] > 0.5) {
                    double g = scale * grad_ih[i][j];
                    nn.weights_ih[i][j] -= g;
                    
                    // Track gradient (with sign for direction changes)
                    nn.grad_history_ih[i][j].push_back(g);
                    if (nn.grad_history_ih[i][j].size() > GRADIENT_HISTORY_SIZE) {
                        nn.grad_history_ih[i][j].pop_front();
                    }
                }
            }
            nn.bias_h[i] -= scale * grad_bh[i];
        ...
        for (int i = 0; i < OUTPUT_SIZE; i++) {
            for (int j = 0; j < HIDDEN_NEURONS; j++) {
                if (nn.mask_ho[i][j] > 0.5) {
                    double g = scale * grad_ho[i][j];
                    nn.weights_ho[i][j] -= g;
                    
                    nn.grad_history_ho[i][j].push_back(g);
                    if (nn.grad_history_ho[i][j].size() > GRADIENT_HISTORY_SIZE) {
                        nn.grad_history_ho[i][j].pop_front();
                    }
                }
            }
            nn.bias_o[i] -= scale * grad_bo[i];
    ...
    void eliminateHighVarianceWeights(NeuralNetwork& nn) {
        // Collect all weights with their gradient variance
        std::vector<std::tuple<double, int, int, int>> variances; // (variance, layer, i, j)
        
        for (int i = 0; i < HIDDEN_NEURONS; i++) {
            for (int j = 0; j < INPUT_SIZE; j++) {
                if (nn.mask_ih[i][j] > 0.5 && 
                    nn.grad_history_ih[i][j].size() >= GRADIENT_HISTORY_SIZE / 2) {
                    double var = calculateVariance(nn.grad_history_ih[i][j]);
                    variances.push_back({var, 0, i, j});
                }
            }
        }
        ...
        for (int i = 0; i < OUTPUT_SIZE; i++) {
            for (int j = 0; j < HIDDEN_NEURONS; j++) {
                if (nn.mask_ho[i][j] > 0.5 && 
                    nn.grad_history_ho[i][j].size() >= GRADIENT_HISTORY_SIZE / 2) {
                    double var = calculateVariance(nn.grad_history_ho[i][j]);
                    variances.push_back({var, 1, i, j});
                }
            }
        }
        ...
        // Sort by variance (highest first = most unstable)
        std::sort(variances.begin(), variances.end(),
            [](const auto& a, const auto& b) { return std::get<0>(a) > std::get<0>(b); });
        
        // Eliminate highest variance weights
        int toEliminate = std::max(1, static_cast<int>(variances.size() * eliminationRate));
        toEliminate = std::min(toEliminate, static_cast<int>(variances.size()) - 4);
        
        for (int i = 0; i < toEliminate; i++) {
            auto [var, layer, row, col] = variances[i];
            if (layer == 0) {
...
TrainingMetrics trainWithVarianceElimination(const std::vector<DataPoint>& trainData,
                                              const std::vector<DataPoint>& testData,
                                              double elimRate, ProgressBar* progress) {
    TrainingMetrics metrics;
    NeuralNetwork nn;
    GradientVarianceTrainer trainer(elimRate);
...
int main() {
    std::cout << "\n";
    std::cout << "╔════════════════════════════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║       NEURAL NETWORK TRAINING #9: GRADIENT VARIANCE ELIMINATION                            ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════════════════════════════════════╝\n\n";
    
    std::cout << "Strategy: Eliminate weights with high gradient variance (unstable)\n";
    std::cout << "  • Track gradient history for each weight\n";
    std::cout << "  • High variance = gradient oscillates = weight can't decide\n";
    std::cout << "  • N=3: Eliminate 33% most unstable (Quaylyn's optimal)\n";
    std::cout << "  • N=9: Eliminate 11% most unstable (conservative)\n";
    std::cout << "  • History window: " << GRADIENT_HISTORY_SIZE << " epochs\n\n";
    ...
    for (int trial = 0; trial < NUM_TRIALS; trial++) {
        ...
        auto n3Metrics = trainWithVarianceElimination(trainData, testData, 
        ...
        auto n9Metrics = trainWithVarianceElimination(trainData, testData, 
    ...
    // Print results
    std::cout << "╔════════════════════════════════════════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║                              RESULTS: GRADIENT VARIANCE ELIMINATION                                    ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "\n  GRADIENT VARIANCE ELIMINATION PRINCIPLE:\n";
    std::cout << "  • Weights with oscillating gradients are \"undecided\"\n";
    std::cout << "  • High variance = noisy signal = unreliable weight\n";
    std::cout << "  • Eliminating unstable weights improves model stability\n";
    std::cout << "  • N=3 (33%): Aggressive stability pruning\n";
    std::cout << "  • N=9 (11%): Conservative pruning\n";
